...
module.exports = {
    ...
    load_universal_cape: function (callback) {
        ...
        try {
            var machine = fs.readFileSync('/proc/device-tree/model');
            if (machine) {
                // Remove non-ASCII characters. The string often contains a trailing null
                // character (\u0000), so this ensures those are removed before comparison. 
                machine = machine.replace(/[^\x00-\x7F]/g, "");
